...
def plot_learning_curve(logfile_dir, result_lists):
    ...
    plt.figure()
    plt.plot(range(1, len(result_lists['train_acc']) + 1), result_lists['train_acc'])
    plt.savefig(logfile_dir + '/train_acc.png')
    plt.close()

    for label in result_lists:
        plt.figure()
        plt.plot(range(1, len(result_lists[label]) + 1), result_lists[label])
        plt.title(label)
        plt.xlabel('Epoch')
        plt.ylabel(label)
        plt.savefig(logfile_dir + '/' + label + '.png')
        plt.close()
    #DONE
